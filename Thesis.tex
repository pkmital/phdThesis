...
To begin either module (i.e. audio and vision), a conceptual framework for unconscious representations in perception will be developed based on theory in behavioral psychology and electrophysiology (Chapter \ref{ch:conceptual-audio} and \ref{ch:conceptual-visual}).  This conceptual framework will then be modeled computationally (Chapter \ref{ch:analysis-audio} and \ref{ch:analysis-visual}) and developed into a scene synthesis framework for the purposes of developing automated collages (Chapter \ref{ch:synthesis-audio} and \ref{ch:synthesis-visual}).  The two modules will later be combined within an augmented reality (Chapter \ref{ch:memory-mosaic}) and film (Chapter \ref{ch:audiovisual}).  Finally, concluding thoughts on the contribution of this work, limitations, and future will be discussed (Chapter \ref{ch:conclusion}).  
...
\item \textit{Attention Model}: Allows regions to be drawn only if the exogenous attention model has a value higher than a fixed threshold.  For our experiments, we set this number to 0.1.  Recall from our computational framework that the attention model also effects the scale of built regions, as the highest point of attention will denote the point of spatial acuity (see Section \ref{sec:visual-acuity}).
...
This chapter develops our previously developed synthesis frameworks for real-time scenarios.

\section{Auditory Memory Mosaicing}

We now turn to the real-time development of auditory scene synthesis, the freely available iOS app \textit{Memory Mosaic}.  This app attempts to develop a scene synthesis experience of sonic ``memories'' which will later be combined with a real-time visual scene synthesis in Section \ref{sec:audiovisual}.  Following our conceptual framework described in Chapter \ref{ch:conceptual-audio}, we must develop event detection in order to understand when an event of sound has occurred within a real-time stream of audio.  Further, another motivation for developing auditory scene synthesis is to be able to take the application and explore different auditory scenes (and not just an office scene), meaning we restricted our development to using a mobile platform.  
