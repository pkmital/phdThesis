...
% Collage appropriates materials.  Information Retrieval allows us to use procedural manipulation of these materials, affording real-time and dynamic collages.   These advances have enabled us to move collage beyond a static medium and into a dynamically generative one.  Where can collage go 
...
%Wiener's seminal text was heavily influenced by other mathematicians at the time.  In particular, Shannon's theory of information helped to formulate one possibly way of measuring information in processes such as matched filters.  Shannon describes a framework of communication starting with a sender, whose information is encoded by a transmitter.  This transmitter allows the sender to represent the information as a physical phenomena, further allowing any receivers capable of understanding the physical phenomena to retrieve the message, and decode it.  Of note are two important 
...
We first discuss background in collage-based practices in Section \ref{sec:background-collage} and information retrieval practices in Section \ref{sec:background-information-retrieval}.  These two fields serve to build a better context of the process of scene synthesis within practice-based domains.  With this background, we will discuss the goals and outline of this thesis in Sections \ref{sec:goals}-\ref{sec:overview}.
...
Use of appropriated materials in GIF: \url{http://scorpiondagger.tumblr.com/}
...
\subsection{Visual Stylization}

Computational approaches towards image manipulation began as filtering and clustering algorithms we applied to images, effectively accentuating regions within an existing image.  These algorithms were also applied to produce stylizations of images, where the aesthetics of the results were associated with e.g., Pointillism \cite{Yang2006,Seo2010}; for cartoonization \cite{Wang2004}, oil and watercolor \cite{Meier1996,Hertzmann2000,Bousseau2007,Gooch2002}, or Impressionism \cite{Litwinowicz1997,Hertzmann1998}).  More recent approaches focused on using user-guided segmentation, where the user manually labels key frames with strokes defining how the frame is stylized (e.g. \cite{O'Donovan2012}) or uses eye-movements in deciding which aspects of a photo are most salient \cite{DeCarlo2002}.
...
\subsection{Sonic Computation}

\hl{Diemo Schwarz's CataRT; Nick Collins's BBCut and klippAV}

Field of Musical Information Retrieval.
...
\end{subfigure}
\caption{3 frames demonstrating datamoshing from Yung Jake's ``Datamosh'' Music Video located at  \url{https://www.youtube.com/watch?v=nS7QvOX8LVk}.  In the first frame, Justin Bieber is displayed.  By removing a keyframe, Yung Jake is able to move Bieber with his own motions.}
\label{fig:datamosh}
\end{figure}
...
You don't have bad Internet, I'm just datamoshing\footnote{Jake is describing how the effect resembles streaming artefacts from slow connections} \\
... \\
I'm on my datamosh, digital effects, man \\
I'm moshing data, geeked up can't stand\footnote{Jake makes mention to being ``on datamosh'' and that he's ``geeked up'' and ``can't stand'', suggesting the effect of data mosh has a drug-like power.} \\
...
\hl{Waveforms of original and mosaic}

\subsection{Discussion}

\hl{Bring it back to the original 4 processes}
...
Equation 3 Ð Optical Flow

where Ix, Iy, and It are the image derivatives, and Vx, and Vy are the components of optical flow in the x (U in vector notation) and y (V in vector notation) direction, respectively.  This method is often referred to as a global method for calculating the optical flow, as the energy constraint (the first term in Equation 3) assumes gray value constancy and does not depend on local image patches [e.g. 97]. The energy equation includes a second term known as the ÒsmoothingÓ term in order to smooth flow where distortions in the flow occur. This leads to better performance at filling in ÒgapsÓ left from moving objects with similarly textured regions than local methods. 
