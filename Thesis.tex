...
\paragraph{Mobile Smartphones} Ubiquitous, embedded computing devices have exploded in the last 5 years.  As a result, the iOS App Store and Android Play have increasingly become platforms for finding works manipulating a user's own media stored on their device.  
...
\label{sec:auditory-memory-mosacing}
\subsection{Introduction}
We now turn to the real-time development of auditory scene synthesis, the freely available iOS app \textit{Memory Mosaic}.  The development on a mobile platform was chosen as one motivation for developing scene synthesis is to be able to explore different scenes.  Developing for a desktop application restricts our scene types to primarily indoor office/bedroom spaces.  We therefore targeted our development towards mobile platforms, meaning the computational power that was offered was also a limitation in our design.  

Following our conceptual framework described in Chapter \ref{ch:conceptual-audio}, we have developed segregation, integration, and template matching thus far in Chapter \ref{ch:analysis-audio}.   We now require that last component of our framework, event detection.  This will allow us to understand when an event of sound has occurred within a real-time stream of audio.  %Another motivation for developing auditory scene synthesis is to be able to take the application and explore different auditory scenes (and not just an office scene), meaning we restricted our development to using a mobile platform.  
...
In order to use our framework in Chapter \ref{ch:synthesis-visual} for real-time settings, we generally had to sacrifice large timesteps and low minimum regions sizes for smaller timesteps and larger minimum region sizes.  Parameters, as described in Section \ref{sec:target-parameters}, are generally set by the user interacting with the process, or contained to a single preset.  Restricting the total number of representations as a first-in-first-out queue also allows the process to continue in real-time with a simple linear search index.  This additional parameter is described within the design of the interface in the next section.
...
\caption{The interface for the visual memory mosaicing program.  There are many options that allow the user to control the synthesis process.  Many have already been described in the context of artistic stylization in Section \ref{sec:artistic-stylization}.  The new ones to discuss here are \textit{Memory Mosaicing}, which toggles between an aggregated database and a pre-defined one and \textit{Max Objects}, which allows one to change the maximum number of proto-objects remembered.}
...
The list of the Top 10 videos of the week are taken from unrulymedia\footnote{\url{http://viralvideochart.unrulymedia.com/all?platform=all&interval=week}}.   The webpage is parsed using a custom C/C++ program looking for the youtube video links:
...
The links file can then be scanned line-by-line looking for the YouTube video identifier as such:
...
Using the collected identifiers, the video can be downloaded using an existing script for downloading videos from YouTube called ``youtube-dl''\footnote{\url{http://rg3.github.io/youtube-dl/}}.  

The number 1 video of the week is to be synthesized after learning the content from the number 2 - 10 videos.  The framework for video synthesis as developed in Section \ref{sec:artistic-stylization} is applied directly in this work, training the number 2 - 10 videos and synthesizing the number 1 video.   We discuss how the parameters for stylization have been chosen in Section \ref{}.  

For audio, we must be able to perform a similar feat as in the visual synthesis, learning from the number 2 - 10 video's audio content, and using this information alone to decode the audio of the number 1 video.  In order to do so, we must extend our auditory scene synthesis work developed in Chapters \ref{ch:analysis-audio} and \ref{ch:synthesis-audio} by incorporating event detection.  We have seen how this works for a real-time setting in Chapter \ref{ch:memory-mosaic}.  In the first weeks of YouTube Smash Up, the same model was applied to producing the auditory scene syntheses.  It however became apparent that more powerful processing could take place, and this was subsequently explored.  In particular, sequence matching, previously employed in \cite{CaseyICMC2007} appeared to provide a more rigorous template matching than the single frame of information our algorithm employed.  
...
In the early weeks, the proto-objects were synthesized using spatial blending.  The feedback received by some people described the result as ``disco balls'', which was for me, an undesirable description.  As a result, after week 3, all syntheses were done without spatial blending.  
