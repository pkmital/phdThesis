...
% mona lisa... 
Inspired by Dada, the anti-art movement by the Situationists emerged by 1957 with sign-scrambling work that would, for instance, change the dialogue from Hong Kong action films or the speech bubbles from comic strips.  They called the technique \textit{d\'{e}tournement} which McLeod notes as being in-between the English translations of ``diversion'' and ``subversion'' and denotes the practice of ``turning culture back on itself'' \cite{McLeod2011a}.   Guy Debord, Situationist theorist and author of \textit{Society of the Spectacle}, discusses the notion of the \textit{spectacle} as the novel mode of social domination or the dominant culture, where promises of fulfillment through entertainment structures were nothing more than deceitful promises \cite{Harold2004} in relation to d\'{e}tournement which describes the practice of devaluing or plagiarizing the spectacle.  The film version of \textit{Society of the Spectacle} incorporates video from various sources and has Debord reading aloud from the original text attempting to subvert the original video content's meaning in a politically fueled process.  The movement would go on to inspire ``culture jammers'' (discussed later in relation to Negativland) and builds an early societal and processual context for \textit{YouTube Smash Up} discussed in Chapter \ref{ch:audiovisual}.
...
Examples of these advances can be seen in Hollywood.  For instance, the opening sequence to Robert Zemeckis' \textit{Forest Gump} (1994) combines a shot of a feather falling against a blue-screen with a shot of a landscape.  The blue-screen allowed special effects company Industrial Light and Magic (ILM) to apply digital post-processing using a threshold, where all values near blue became transparent, and the remaining values added or blended to the landscape video.  The final effect produces ``a new kind of realism, which can be described as 'something which looks is intended to look exactly as if it could have happened, although it really could not''' \cite{LevManovich1996}.  \hl{do we need it?}  ILM, owned by The Walt Disney Company which acquired the company when buying LucasFilm in 2012, are also responsible for earlier films such as \textit{Star Wars} (1974), to more recent ones including \textit{Jurassic Park}, the entire series of \textit{Harry Potter}, and \textit{Avatar}, amongst nearly 300 others.  In many of these films, compositing afforded an entirely new type of collage process, where computer animations or models could be placed within another scene (possibly also computer generated).  
