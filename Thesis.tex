...
As collage-based practices have progressed over the last 100 years, media too has evolved.  As a result, the tools and technologies available for manipulating and accessing media have altered the practice.  Specifically, within computational practices, procedural manipulation of media content have allowed artists to invoke new ways of seeing through algorithmic processing of the media.  

For instance, Nick Collin's BBCut allows one to generate computationally generated audio collages of existing audio tracks.  His software automatically re-assembles splices of the original audio track in order to emulate different musical styles such as breakbeat or thrashbeat \cite{Collins2004}.  

The inspiration this thesis takes from such practices is the recombination from the perspective of the participant rather than the audience.  That is, a participant's prior experiences with the original content of the collage affords them to create interpretations of the collage.  In other words, the collage is a juxtaposition of a familiar material, rather than an arbitrary one.  As a result, there are expectations, surprises, and strange combinations that give rise to new meanings.
...
Communication theory... signal/noise... receiver... relationship of randomness to information, relationship to exogenous attention or change detection within perception... }

Feedback as regulatory process, would self-regulate, though financial collapse may have disproven the notion.
...
Each new segment detected is written to disk using Apple's Extended Audio File Format.  Only the audio segment's first frame's 89-dimensional LFCC feature vector is retained in memory in order to form a matrix of vectors.  \hl{As our model is based on attentional shifts,}
