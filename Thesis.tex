...
Other techniques in glitch-based art include ``pixel sorting'' (coined by Kim Asendorf\footnote{\url{http://www.kimasendorf.com/about/README}}), where pixels in an image are spatially sorted based on their values, or opening JPEG or other similarly encoded files in a normal text-pad and rewriting parts of it using normal keyboard letters.  This intermediate representation (i.e. the one viewed in the textpad) is not meant to be decoded by a text parser.  However, the interaction of writing text within this format is then stored back into the file, and when read by a decoder, viewed as an image.  The parts that were changed are likely unknown by the one manipulating the file, so there may also be a trial-and-error process.  

\paragraph{NPAR}

The field of non-photorealistic artistic rendering (NPAR) refers to computer graphics techniques focused on visual communication.  Until now, we've already seen a wide adoption of NPAR techniques by visual artists using existing software (see Section \ref{sec:technological-developments}).   These techniques are commonly found adopted into existing software such as 

The field began as filtering and clustering algorithms were applied to images, accentuating regions within an existing image to produce renderings associated with different styles (e.g., for Pointillism \cite{Yang2006,Seo2010}; for cartoonization \cite{Wang2004}; for oil and watercolor \cite{Meier1996,Hertzmann2000,Bousseau2007,Gooch2002}; for Impressionism \cite{Litwinowicz1997,Hertzmann1998}).  More recent approaches focused on using user-guided segmentation, where the user manually labels key frames with strokes defining how the frame is stylized (e.g. \cite{O'Donovan2012}) or uses eye-movements in deciding which aspects of a photo are most salient \cite{DeCarlo2002}.

Hertzmann's seminal work in Image Analogies \cite{Hertzmann2001} presented a branch from the aforementioned approaches by allowing control of the rendering process through choosing a pair of example images.  By finding the correlated textures/patterns between image A to another image A', a user could then stylize a target image B by analogy into B' (later extended to include analogies between curved strokes \cite{Hertzmann2002}).  In the same year, \cite{Efros2001,Liang2001a} also developed methods in texture transfer and patch-based sampling, where existing image material was used to synthesize textures of arbitrary sizes.  These methods were later extended in \cite{Wang2004a}, where a user specified small blocks in an example painting that represented the aethestics to recreate.  These blocks were then synthesized along computed paint strokes in the target image using an efficient hierarchical texture synthesis method.  

Wang's approach as well as more recent methods (e.g., \cite{Guo2006}) rely on user interaction to select the representative patches expressing an artistic style.  As well, most of these approaches (e.g., texture transfer or \cite{Lee2010}) all rely on a single source image in order to transfer the style of the texture, meaning the range of possible fragments are constrained to the information contained in a single image.  In this thesis, we develop a similar approach but which does not require the user to manually label any regions and that is not confined to a single example image while still affording a range of possible styles (see Section \ref{sec:artistic-stylization}).  
...
Texture Synthesis \cite{Lewis1984,Haberli1993}

\hl{Computational dictionary-based approaches for image stylization} \cite{Zeng2009,Healey2004}.  

\hl{Collage/mosaic-based work allowing a user to work with a period of an artist's work or entire videos, for example} \cite{Kim2002,Orchard2008,Huang2011a,Miller2012}.  

\cite{Huang2011a} \hl{describes an approach that is also motivated by an artist making use of collage.  Their approach produces what they call ``Arcimboldo-like'' collages in the style of 18th century painter Giuseppe Arcimboldo, relying on user strokes to segment the images used.}

\hl{Also} \cite{Chang2010}.
...
\section{Introduction to Auditory Scene Synthesis}
...
\subsection{Related Work in Visualizing Audio Archives}
...
\subsection{Methods for Visualization}
...
\subsection{Graphical User Interface of the Browser}
...
\item \textbf{Proto-Objects} Finally, the description of the scene can be defined by a collection of volatile units.  These will likely be described by color and shape-based representations taking into account the coarser resolutions outside of fixation due to visual acuity.  Further, the encoding should not be a color space where color are given equal weighting, but based on the response properties of cones versus rods.
\end{enumerateb}
...
%Behavioral evidence from change blindness studies has indicated an essential gap in our encoding of a scene, namely the precise details of a scene are only encoded at the point of fixation.  This is likely due to the computational complexity required for encoding an entire scene's high-spatial resolution information.  More importantly, it is physiologically impossible given the visual acuity limitations of the retina.  

In this Chapter, we build a computational implementation of the conceptual framework built in Chapter \ref{ch:conceptual-visual}.  This model aims to encode any number of visual scenes, describe them using the representations motivated in our conceptual framework in Chapter \ref{ch:conceptual-visual}, and use them for decoding any target visual scene.  For now, we simply describe the computational elements required for encoding and decoding.  In Chapter \ref{ch:synthesis-visual}, these procedures will be used within a collage-based arts practice to open a dialogue surrounding the nature of perception and finally combined with a similar model of audio in Chapter \ref{ch:audiovisual}.  

\section{Related Work}

Similar computational models have been developed to explain visual perception machinery. (e.g. \cite{Walther2006,Orabona2007a}).   They each suffer from a number of problems: (1) they lack the inclusion of the evidence of the response properties of photoreceptors in the retina as there is no indication of the current or ongoing attention within the visual scene; (2) they infer context based on solely a static image whereas the real-world is dynamic; and (3), they cannot distinguish groupings of proto-objects and instead create discrete maps which are thresholded as attention or saliency maps.  Furthermore, the interest in the previously cited models of visual perception is in predicting attention towards a scene, rather than simulating aspects of perception within an arts-based practice.
...
In this section, we develop our computational implementation of a mid-level object representation denoted as proto-objects.  Following our conceptual framework in Chapter \ref{ch:conceptual-vision}, this representation will be described by shape, color, and luminance.  Moreover, these must be capable of describing shapes invariant to location, scale, and rotation.  For this purpose, we make use of maximally stable color regions (MSCR) \cite{Forssen2007}.   The input to this algorithm will be the image described by our visual acuity map.  

\subsubsection{Detection}

The algorithm described in \cite{Forssen2007} uses agglomerative clustering to group similar neighboring pixels.   Similarity is described by a distance measure which takes into account the inherent camera noise and the probability distribution of each RGB color channel.  Considering this distance manifold, the algorithm first applies multiple thresholds at fixed distance intervals.  Regions are denoted as maximally stable if they do not grow larger than a minimum area rate after a fixed number of time-steps.  The algorithm can also be thought of in terms of level sets, where each level set describes slices of the manifold described by the distance function.  In either case, this algorithm is capable of determining an implicit ordering of coarse-to-fine regions, where a larger cluster can consist of many smaller ones. 

Previous techniques employing posterization, filtering, or watershed have had to apply their algorithm at multiple scales in order to discover regions that are superimposed or overlapped, increasing their computational complexity.  MSCR therefore benefits over these previous techniques as it provides an implicit ordering of superimposed regions discovered through successive time-steps of the clustering algorithm.  Further, it allows us to prune regions by restricting their area to a range of minimum and maximum sizes.  This will be desirable when applying synthesis for artistic purposes, as we do in Chapter \ref{ch:synthesis-visual}.

We use MSCR to detect the set of all regions in either candidate or target frames, denoted as $\mathbf{R_C} = \{R_1, R_2, ..., R_{N_C}\}$ and $\mathbf{R_T} = \{R_1, R_2, ..., R_{N_T}\}$ where $N_C$ is the number of regions detected in all candidate frames and $N_T$ is the number of target regions.  

\subsubsection{Tracking}

We use our attentional map to indicate whether a new blob has disappeared.  This coincides with Rensink's notion of a ``volatile'' unit, as we do not wish to have blobs persist forever, but rather that they are replaced due to changes in the visual world (e.g. as discussed in Section \ref{} when looking at Change Blindness).

To model volatility and therefore updates in our proto-objects, we only re-detect proto-objects when a blobs average attentional value exceeds a threshold.  Using Equation \ref{eq:attention} as our attentional map, and $\mathbf{R}$ as the set of proto-objects currently detected, we redirect proto-objects within an already detected proto-object if:

\begin{equationb}
\frac{1}{N_{x}N_{y}}\sum_{R_x}^{N_{x}}\sum_{R_y}^{N_{y}}p(R_x,R_y) \; > \; \tau
\end{equationb}

where $N$ is the number of pixels in the proto-object $R$, $R \subset \mathbf{R}$, $R_{x}$ and $R_{y}$ are short hand for the pixel locations of the indexed region, and $0.0 \, \leq \, \tau \, \leq \, 1.0$ is a small threshold, often set to $0.05 - 0.10$.  This means our attentional map indicates where to recompute a proto-object.   Practically, this produces temporal coherence across frames, meaning proto-objects that aren't being attended to do not need to be recompute.
...
This chapter uses the computational framework developed in Chapter \ref{ch:analysis-visual} and explores three practical developments of it within the context of visual scene synthesis: (1) in Section \ref{sec:artistic-stylization}, an automated method for artistically stylizing an image or video (e.g. using representations in a corpus of Van Gogh paintings to stylize a home video); (2) in Section \ref{sec:photosynthesizer}, an iOS application \textit{PhotoSynthesizer} which allows one to use visual scene synthesis with the photos from their mobile device; and (3), in Section \ref{sec:visual-memory-mosaicing}, a real-time visual scene synthesis which learns visual representations from the same scene it attempts to synthesize (visual memory mosaicing).  The first two outputs can be seen as experiments in leading to the final output of visual memory mosaicing.  This process is modeled similarly to audio memory mosaicing (discussed in Section \ref{sec:audio-memory-mosaicing}), and will be combined with it in the final practical chapter of this thesis in Chapter \ref{ch:audiovisual}.
...
\caption{An example of visual memory mosaicing.  The content is coarsely re-synthesized, sacrificing smaller blobs to produce a more abstract almost cubist synthesis.}
\end{figure}
...
\end{quotationb}
\begin{quotationb}
Instructions say 'pick a few pictures'. Four pictures: 'insufficient memory'. Three pictures: 'insufficient memory'. Two: 'insufficient memory'.... (I don't think you can 'synthesise' [\textit{sic}] fewer than two things??). Guess I'll wait for the update (it's coming, right?)
\end{quotationb}
\begin{quotationb}
