...
Extending this research into a basic computational model requires at least: (1), an onset detector such as evidenced by the N1; (2), a template match functionality as evidenced by the PN; and (3), an online segregation as evidenced by the MMN and ORN.  Of course, this is a radical simplification of the perceptual system of auditory listening.   Of course, this is a gross simplification, and an idealized model should consider, at the very least: the numerous influences of attention (e.g. \cite{Shamma2011}), how integration occurs and if it should occur before or after segregation (e.g. \cite{Sussman2005}), what a template is defined by, how the stimulus timings, inter stimulus timings, and contextual influences effect stream formation, and how binaural cues such as interaural level and time differences may effect any one of these processes.  However, any one of these questions are all very active areas of research.  As a result, the basic set of components laid above can at least provide a starting point for a plausible model from which representations of unconscious auditory objects, or streams, may be built.  

The next sections are dedicated to expanding on the conceptual underpinnings of each component further.

\subsection{Onset Detection}

Evidence demonstrating the elicitation of an N1 ERP component suggests that as little as 60-70 ms are required before the brain is capable of detecting the onset of a stimulus.  This component demonstrates the brains remarkable ability to detect a change in the environment even before we become aware of it.  It takes almost another 250-300 ms before we can become conscious aware of the stimulus onset, i.e. report the actual stimulus onset, as demonstrated by the late positive ERP component, the P3.  As a result, it seems plausible that a basic perceptual model should have the capability of detecting stimulus changes in the environment.  

However, the literature presented thus far does not seem to provide enough evidence on how an onset may be computationally modeled.  In each of the experiments reviewed above, short evoked tones were presented to a participant, thus assuming that each tone would provide an event evoking an N1 response.  However, the world is not neatly aligned by short evoked tones, nor events.  Rather, it is a continuous amalgamation that our brain is capable of neatly organizing.  How then does the N1 decide when an event has occurred?  

Looking the evidence of the elicitation of the MMN and the groundwork laid by Bregman's Auditory Scene Analysis, one may be able to begin to define an onset detector based on similar strategies of simultaneous and sequential grouping cues.  Though these are traditionally thought to provide evidence of segregation in a continuous stream, they can similarly be thought of in terms of a segregation from an ongoing stream into one that may be further processed.  As a result, the MMN and N1 may share very similar basic strategies. The major difference seems to be that the MMN is time locked to the onset, providing a basis for separation, i.e. the sonic environment before and after the N1 response.  The N1 may likely be based on an automatic time window, where the capability for detecting irregularities of the sonic world are based on task/attentional demands.  Further, given the research demonstrated thus far, it is likely that this window is very short, e.g. within the timespan of evoking P3s, given the capability for detecting ISIs of up to 350 ms.  
