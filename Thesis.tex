...
This chapter has discussed the real-time development of scene synthesis leading to the exhibition of \textit{Augmented Reality Hallucinations}.  This work placed a user within augmented reality goggles and headphones in order to intercept the light and sound coming to them.  The light as captured by a camera and sound as captured by microphone was then synthesized using an aggregated form of scene synthesis that does not use a predefined database, memory mosaicing.  In this case, the database is instead grown over time based on what has been previously detected.  This learning algorithm is restrictive, however.  For vision, it will only learn shapes and colors far enough away from the ones it has already learned (as seen in the keyboard in Figure \ref{fig:augmented-reality-keys}).  For sound, similarly, the shape of the sound as measured by the MFCCs determine the whether or not a sound is encoded and used for future decodings.  
...
This chapter presented the work of \textit{YouTube Smash Up}, a short film series producing audiovisual scene syntheses of the number 1 YouTube video using the remaining top 10 videos.  The work revealed aspects of YouTube's ContentID system which attempts to automatically detect infringing videos.  This system mistakenly identified many of my smash up videos as infringing.  

Though it is difficult to describe an artwork's result, as may be necessary within the context of a practice-based PhD, some result may be described by the work being sought out by curators.  Having been invited to the Media Art Histories 2013 conference in particular, as part of the ART+COMMUNICATION exhibition offered a unique perspective on the work.  
...
Witnessing a shift between the role of scientist and artist.  Seen in increased collaborations between the two, and the roles within either becoming less clearly defined.  Noticeable in IEEE VISART, ACM MM, SIGGRAPH, IEEE ISMAR all including arts programs as part of their conference.  The birth of the term ArtScience
