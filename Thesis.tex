...
%digital form interaction within the process has afforded interesting new \hl{Work on talking about all of this stuff:} Zils and Pachet's Musaicing, Diemo Schwarz's CataRT, Ari Lazier's Mosievius \cite{Lazier2003a}, Michael Casey and Michael Grierson's Soundspotter/REMIX-TV \cite{CaseyICMC2007}, Michael Grierson's Plundermatics, Nick Collins' and Frederik Olofsen's KlippAV, Collins and Sturm's Dictionary Based Methods.... 
...
\hl{Ben Bogart and Philippe Pasquier's Context Machines, Ben Bogart's Dreaming Machine}
...
%Computational dictionary-based approaches (\cite{Zeng2009,Healey2004}) for image stylization.  

%Collage/mosaic-based work (e.g. \cite{Kim2002,Orchard2008,Huang2011a,Miller2012}), allowing a user to work with a period of an artist's work or entire videos, for example.  

%\cite{Huang2011a} describes an approach that is also motivated by an artist making use of collage.  Their approach produces what they call ``Arcimboldo-like'' collages in the style of 18th century painter Giuseppe Arcimboldo, relying on user strokes to segment the images used. 
...
\caption{Left to right: (a) High pass flicker (Flicker); (b) Low pass flicker (Flicker-N); (c) Horizontal optical flow (U-Flow); (d) Vertical optical flow (V-Flow) for the frame in Figure 1(a)}
...
\hl{Shape recognition. Skeletonization. Level sets, Boundary value problem. Agglomerative clustering. }

\hl{Acuity map -> Region detection -> Shape representation}

\hl{Rewrite following sections within general context and not artistic stylization}
...
\hl{Talk about the different sections}
...
\subsubsection{Week 1 | Justin Bieber - As Long As You Love Me ft. Big Sean}
...
\subsubsection{Week 2 | Call Me Maybe - Carly Rae Jepsen (Chatroulette Version)}

\begin{enumerateb}
#2 La Educaci&oacute;n Prohibida - Pel&iacute;cula Completa HD \\
#3 Carly Rae Jepsen - Call Me Maybe \\
...
\subsubsection{Week 3 | Call Me Maybe - Carly Rae Jepsen}
\begin{enumerateb}
#2 Flo Rida - Whistle [Official Video] \\
...
\subsubsection{Week 4 | 11 Month Old Twins Dancing to Daddy's Guitar}
\begin{enumerateb}
...
\subsubsection{Week 5 | PSY - GANGNAM STYLE (?????) M/V (YouTube SmashUp)}
\begin{enumerateb}
#2 ADELE - Skyfall \\
...
\subsubsection{Week 6 | Miley Cyrus - Wrecking Ball}
\begin{enumerateb}
