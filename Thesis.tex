...
This thesis has investigated scene synthesis, an approach for computationally generating collages.  Specific to this collage process is the use of psychologically motivated pre-attentive representations thought to support our ongoing perception of the world.  The objective of this process was to explore the ability of a computationally generative collage process to produce new meanings, as well as to explore the meaning making process within our own perceptual systems.  We attempted to do so by placing emphasis on the representation of pre-attentive units within the collage process, i.e. the representations we are likely to encode, yet that we are not aware of.  These units were conceptualized and developed modularly, where audio was developed independently of vision, and only in the later chapters, combined, though still kept as independent frameworks.  

For audio, we discussed the notion of the auditory event as a possible unit of representation in Chapter \ref{ch:conceptual-audio}.  This representation was motivated after reviewing presiding theories in cognitive neuroscience and experimental psychology such as streaming paradigms and event-related potentials such as the mismatch negativity.  These theories placed emphasis on the importance of temporal irregularities in the ongoing auditory scene in order to detect events and discretize the continuous amalgamation of sounds where updates in representations or selective attention processes may occur.  After reviewing this literature, we were able to develop a conceptual framework that could be implemented computationally.  This framework encompassed four processes: \textit{event detection}, as demonstrated by various event-related potentials: the N1 component, \textit{matching}, as evidenced by the PN component, \textit{segregation}, as evidenced by the MMN and ORN components, and finally \textit{integration}, the encoding process which stores the result of the event detection.  The latter three components were developed computationally in Chapter \ref{ch:
